...
flags.DEFINE_string('model_type', 'bdqn', 'Enter the model type.')
FLAGS = flags.FLAGS
...
def main(argv):
    ...
    if FLAGS.model_type == 'dqn_ids':
        ...
        bdqn = b_deep_q_networks.DeepBBQNetwork(hparams=hparams,
                                                 ipt_shape=(hparams.batch_size, hparams.fingerprint_length + 1),
        ...
        if FLAGS.hparams is not None:
            with gfile.Open(FLAGS.hparams, 'r') as f:
                hparams = b_deep_q_networks_ids.get_hparams(**json.load(f))
        else:
            hparams = b_deep_q_networks_ids.get_hparams()

        environment = Molecule_IDS(atom_types=set(hparams.atom_types),
                               init_mol=FLAGS.start_molecule,
                               allow_removal=hparams.allow_removal,
                               allow_no_modification=hparams.allow_no_modification,
                               allow_bonds_between_rings=hparams.allow_bonds_between_rings,
        ...
        bdqn = b_deep_q_networks_ids.DeepBBQNetwork(hparams=hparams,
                                                    ipt_shape=(hparams.batch_size, hparams.fingerprint_length + 1),
                                                    local_reparameterisation=hparams.local_reparam,
                                                    optimizer=hparams.optimizer,
                                                    grad_clipping=hparams.grad_clipping,
