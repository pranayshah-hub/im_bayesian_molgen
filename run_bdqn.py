...
def run_training(hparams, environment, bdqn):
    ...
    with tf.Session() as sess:
        ...
        for episode in range(hparams.num_episodes):
            ...
            # Update the Thompson network every thomp frequency
            if hparams.thomp_freq == 20:
                if (episode + 1) % hparams.thomp_freq == 0:
                    sess.run(bdqn.update_op2)
...
def _episode(environment, bdqn, memory, episode, global_step,
    ...
    """Runs a single episode.
      ...
      Args:
        ...
        memory_rbs: Pre-filled ReplayBuffer from previously trained models.
        ...
        kl_weight_schedule: Schedule used to decay KL loss term,
            if defined outside this function.
        sequence_buffer: Deque to store optimization path of episode
        sn1, sn2: Integers. Randomly sampled numbers for a Thompson
            Sampling frequency of half an episode.
    ...
    for step in range(hparams.max_steps_per_episode):
        ...
        if (episode > min(50, hparams.num_episodes / 10)) and (global_step % hparams.learning_frequency == 0):
            ...
            # episode_tensor = bdqn.get_episode(tf.Tensor(episode, [], tf.int32))
...
def _step(environment, bdqn, memory, episode, hparams, exploration, head):
    """Runs a single step within an episode.
      ...
      Args:
        ...
        memory: ReplayBuffer used to store observations and rewards.
        memory_rbs: Pre-filled ReplayBuffer is Replay Buffer Spiking is used.
        episode: Integer episode number.
        hparams: HParams.
        exploration: Schedule used for exploration in the environment.
        head: Integer index of the DeepQNetwork head to use.
        sample_number1, sample_number2: Integers. Randomly sampled numbers for a
            Thompson Sampling frequency of half an episode.
