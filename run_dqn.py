...
flags.DEFINE_string('model_dir',
                    # '/vol/bitbucket/pds14/bsdqn/56lr4_0.9',
                    '/data/PycharmProjects/B2Q_code/dqnbs_allrings_moreatoms',
...
def _episode(environment, dqn, memory, episode, global_step, hparams,
             summary_writer, molecule_monitor, exploration, beta_schedule):
    ...
    for step in range(hparams.max_steps_per_episode):
        ...
        if step == hparams.max_steps_per_episode - 1:
            ...
            # episode_summary = dqn.log_result(result.state, result.reward)
            # summary_writer.add_summary(episode_summary, global_step)
            #wandb.log({'episode_summary': episode_summary, 'global_step': global_step})
        ...
        if (episode > min(50, hparams.num_episodes / 10)) and (global_step % hparams.learning_frequency == 0):
            ...
            #wandb.log({'episode_summary': error_summary, 'global_step': global_step})
...
def _step(environment, dqn, memory, episode, hparams, exploration, head):
    ...
    observations = np.vstack([np.append(deep_q_networks.get_fingerprint(act, hparams), steps_left) for act in valid_actions])
    ...
    steps_left = hparams.max_steps_per_episode - environment.num_steps_taken
    action_fingerprints = np.vstack([np.append(deep_q_networks.get_fingerprint(act, hparams), steps_left) for act in environment.get_valid_actions()])
