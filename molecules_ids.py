...
class Result(
    collections.namedtuple('Result', ['state', 'reward', 'reward_adjusted', 'ids_term', 'terminated'])):
    ...
    The namedtuple contains the following fields:
      ...
      ids_term: Float. The Information-based term used to guide exploration via the reward function.
...
def get_valid_actions(state, atom_types, allow_removal, allow_no_modification,
  ...
  """
    ...
    atom_valences = {
        atom_type: molecules_py.atom_valences([atom_type])[0]
...
class Molecule(object):
    ...
    Args:
      ...
      num_obj: Integer. Number of training objectives.
    """
        ...
        self._max_new_bonds = dict(
            list(zip(atom_types, molecules_py.atom_valences(atom_types))))
    ...
    def step(self, action, ids_score=None, ids_score_norm=None, ids_score_2=None, ids_score_norm2=None,
             ids_true_norm=None,
    ...
    """
        ...
        if self._num_obj == 2:
            if ids_score is not None:
                if lr and not multi_obj:
                    self.ids_score = np.tanh(ids_score * 1e-3)  # 1e-2
                else:
                    self.ids_score = np.tanh(ids_score * 1e-5) # bdqn_ids
                    # self.ids_score = np.tanh(ids_score * 1e-2)  # dqn_ids
                r = r_0 - self.ids_score
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.ids_score,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif ids_score_norm is not None:
                if ids_score_norm > 1:
                    ids_score_norm = 1.0
                self.ids_score_norm = np.tanh(ids_score_norm)  # No scale factor required if using tanh
                r = r_0 - self.ids_score_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.ids_score_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif ids_score_2 is not None:
                self.ids_score_2 = np.tanh(ids_score_2 * 1e-8)  # No scale factor required if using tanh
                r = r_0 - self.ids_score_2
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.ids_score_2,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif ids_score_norm2 is not None:
                self.ids_score_norm2 = np.tanh(ids_score_norm2)  # No scale factor required if using tanh
                r = r_0 - self.ids_score_norm2
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.ids_score_norm2,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif ids_true_norm is not None:
                self.ids_true_norm = np.tanh(ids_true_norm)
                r = r_0 - self.ids_true_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.ids_true_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif ids_true_e1_norm is not None:
                if ids_true_e1_norm > 1.0:
                    ids_true_e1_norm = 1.0
                self.ids_true_e1_norm = np.tanh(ids_true_e1_norm)
                r = r_0 - self.ids_true_e1_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.ids_true_e1_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif info_gain is not None:
                self.info_gain = np.tanh(info_gain * 1e4)
                r = r_0 + self.info_gain
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.info_gain,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif info_gain_norm is not None:
                self.info_gain_norm = np.tanh(info_gain_norm)
                r = r_0 + self.info_gain_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.info_gain_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif info_gain_true_norm is not None:
                self.info_gain_true_norm = np.tanh(info_gain_true_norm)
                r = r_0 + self.info_gain_true_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.info_gain_true_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif info_gain_true_e1_norm is not None:
                self.info_gain_true_e1_norm = np.tanh(info_gain_true_e1_norm)
                r = r_0 + self.info_gain_true_e1_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                ...
                if e1_norm > 1:
                    e1_norm = 1.0
                self.e1_norm = np.tanh(e1_norm)
                r = r_0 + self.e1_norm # -
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.e1_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif e2 is not None:
                self.e2 = np.tanh(e2 * 1e3)
                r = r_0 - self.e2
                result = Result(
                    state=self._state,
                    reward=r_0,
                    ...
                    ids_term=self.e2,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            elif e2_norm is not None:
                if e2_norm > 1:
                    e2_norm = 1.0
                self.e2_norm = np.tanh(e2_norm)
                r = r_0 - self.e2_norm
                result = Result(
                    state=self._state,
                    reward=r_0,
                    reward_adjusted=r,
                    ids_term=self.e2_norm,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            return result
        elif self._num_obj == 3:
            if ids_score is not None:
                self.ids_score = -1 * (np.tanh(ids_score * 1e-4))
                r = (*r_0, self.ids_score)
                result = Result(
                    state=self._state,
                    reward=r,
                    reward_adjusted=None,
                    ids_term=self.ids_score,
                    terminated=(self._counter >= self.max_steps) or self._goal_reached())
            return result
